...
function UserSelect() {
  ...
  const registerUser = async () => {
    console.log(name,imgUrl,imgObject,descriptors)
    try {
      const response = await axios.post("https://face-backend-eia8.onrender.com/api/v1/users/register", {
        username: name,
        imgUrl: imgUrl,
    ...
    <div className="h-full flex flex-col items-center justify-center w-full max-w-[720px] mx-auto">
      ...
      {loadingModels ? (
        ...
        <>
          ...
          {registrationSuccess && <p>User registration successful!</p>}
        </>
      )}
    </div>
  );
